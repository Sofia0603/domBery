...
<div class="detail-1">
    <div class="form-row align-items-stretch mb-lg-4">
        <div class="col-12 col-lg mb-4 mb-lg-0">
            ...
            <h1 class="mb-2"><?=trim($arResult['DISPLAY_PROPERTIES']['DETAIL_TITLE'] ? $arResult['DISPLAY_PROPERTIES']['DETAIL_TITLE']['DISPLAY_VALUE'] : $arResult['NAME'])?><a
                        ...
                        title="Добавить в избранное"><!--i class="fal fa-star font-26"></i--></a>
            </h1>
            <div class="d-flex flex-row">
            <? if ($arResult['DISPLAY_PROPERTIES']['ONELINE_AREA']) { ?>
                <div class="gray mr-3"><?=$arResult['DISPLAY_PROPERTIES']['ONELINE_AREA']['NAME'].': '.$arResult['DISPLAY_PROPERTIES']['ONELINE_AREA']['DISPLAY_VALUE']?>м²</div>
             <? } ?>

             <? if ($arResult['PROPERTIES']['D_ONELINE_FLOOR']) { ?>
                 <div class="gray mr-3"><?=$arResult['PROPERTIES']['D_ONELINE_FLOOR']['NAME'].': '.$arResult['PROPERTIES']['D_ONELINE_FLOOR']['VALUE']?></div>
             <? } ?>

             <? if ($arResult['DISPLAY_PROPERTIES']['SIZE']) { ?>
                 <div class="gray mr-3"><?=$arResult['DISPLAY_PROPERTIES']['SIZE']['NAME'].': '.$arResult['DISPLAY_PROPERTIES']['SIZE']['DISPLAY_VALUE']?></div>
              <? } ?>

             <? if ($arResult['DISPLAY_PROPERTIES']['SERIES']) { ?>
                 <div class="gray mr-3"><?=$arResult['DISPLAY_PROPERTIES']['SERIES']['NAME'].': '.$arResult['DISPLAY_PROPERTIES']['SERIES']['DISPLAY_VALUE']?></div>
              <? } ?>
	...
	<div class="form-row align-items-start border-bottom border-dashed pb-4 my-4 justify-content-between justify-content-md-start">
        ...
        <div class="col-12 col-sm-6 col-lg-auto mb-3 mb-lg-0">
            <a class="btn btn-lg btn-primary btn-block" href="#book" data-fancybox >Заказать дом "под ключ"</a>
        </div>

        <div class="col-12 col-sm-6 col-lg-auto mb-3 mb-lg-0">
            <a class="btn btn-lg btn-light btn-block" href="#preview" data-fancybox >Хочу внести изменения</a>
        </div>
...
<?php
$hasAnyProperty = !empty($arResult['PROPERTIES']['EQUIOMENT_BOX']['VALUE'])
    || !empty($arResult['PROPERTIES']['EQUIOMENT_CONTOUR']['VALUE'])
    || !empty($arResult['PROPERTIES']['EQUIOMENT_PRE_CLEANING']['VALUE'])
    || !empty($arResult['PROPERTIES']['EQUIOMENT_COME_AND_LIVE']['VALUE']);
...
<div class="mt-3">
    <table class="table table-bordered table-striped mb-4">
        ...
        <tbody>
            <?php if (!empty($arResult['PROPERTIES']['EQUIOMENT_BOX']['VALUE'])): ?>
            <tr>
                <td>Коробка</td>
                <td><?= $arResult['PROPERTIES']['EQUIOMENT_BOX']['VALUE'] ?></td>
            </tr>
            <?php endif; ?>

            <?php if (!empty($arResult['PROPERTIES']['EQUIOMENT_CONTOUR']['VALUE'])): ?>
            <tr>
                <td>Тёплый контур</td>
                <td><?= $arResult['PROPERTIES']['EQUIOMENT_CONTOUR']['VALUE'] ?></td>
            </tr>
            <?php endif; ?>

            <?php if (!empty($arResult['PROPERTIES']['EQUIOMENT_PRE_CLEANING']['VALUE'])): ?>
            <tr>
                <td>Предчистовая</td>
                <td><?= $arResult['PROPERTIES']['EQUIOMENT_PRE_CLEANING']['VALUE'] ?></td>
            </tr>
            <?php endif; ?>

            <?php if (!empty($arResult['PROPERTIES']['EQUIOMENT_COME_AND_LIVE']['VALUE'])): ?>
            <tr>
                <td>Заезжай и живи</td>
                <td><?= $arResult['PROPERTIES']['EQUIOMENT_COME_AND_LIVE']['VALUE'] ?></td>
            </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<? if ($arResult['DETAIL_TEXT']) { ?>
	<div class="my-5 pb-3">
		...
		<div class="detail-text-dombery text-limitation font-18" data-height="600">
...
<div id="preview" class="popup" style="display: none;">
    ...
    <div class="darkgray-bg rounded-lg px-3 pt-3">
        <form class="form-ajax form-row justify-content-center" action="/local/include/ajax-feedback-dombery.php">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h5 class="modal-title">Получить пример сметы</h5>
      ...
      <form id="smetaForm">
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>Телефон *</label>
            <input type="text" name="PHONE" class="form-control phone-mask" required>
          </div>
          ...
          <input type="hidden" name="PROJECT" value="<?= $arResult['NAME'] ?>">

          <div class="form-check mt-3">
            <input class="form-check-input" type="checkbox" id="agree" required>
            <label class="form-check-label" for="agree">
              Я согласен с <a href="/privacy-policy/" target="_blank">политикой обработки ПДн</a>
            </label>
          </div>
