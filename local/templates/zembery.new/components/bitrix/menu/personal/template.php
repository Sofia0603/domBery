...
<?if (!empty($arResult)):?>
	...
	<ul class="menu personal-menu">
		...
		?>
			<li class="<?=($arItem["SELECTED"] ? 'active':'')?>" ><a href="<?=$arItem["LINK"]?>" class="<?=($arItem["SELECTED"] ? 'active':'')?> <?=$arItem["PARAMS"]['class']?>" <?=$arItem["PARAMS"]['params']?> ><?=$arItem["TEXT"]?></a>
		<?endforeach?>
