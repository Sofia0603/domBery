...
if ($arResult['DISPLAY_PROPERTIES']['GEOPOINT']) {
    ...
    $arResult['DISPLAY_PROPERTIES']['GEOPOINT_SHOT'] = array(
        'NAME' => 'Координаты GPS',
        'DISPLAY_VALUE' => '<a href="http://maps.yandex.ru/?ll='.$arCoord[1].'E,'.$arCoord[0].'N&z=14&l=sk,&pt='.$arCoord[1].'E,'.$arCoord[0].'N,pmwtm1" target="_blank">'.round($arCoord[0],6) . ', ' . round($arCoord[1],6) . '<i class="fal fa-crosshairs ml-2 green"></i></a>'
    );
}
...
//if (empty($arResult['PROPERTIES']['LANDING_MORE_PHOTO']['VALUE'])) $arResult['PROPERTIES']['LANDING_MORE_PHOTO']['VALUE'] = $arResult['PROPERTIES']['MORE_PHOTO']['VALUE'];
