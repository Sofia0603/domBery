...
<tr class="file-inline-file" id="wd-doc#element_id#">
	<td class="files-name">
		<span class="files-text">			
			<a href="#src#" target="_blank" class="f-wrap" data-role='name'>#name#</a>
		</span>
	</td>
...
{
?>
<a href="javascript:void(0);" id="file-selectdialogswitcher-<?=$uid?>" class="file-selectdialog-switcher btn btn-light" <?
	?>onclick="BX.onCustomEvent(this.parentNode, 'BFileDLoadFormController');return false;"><span><?
		?><?=($arParams["TITLE_UPLOAD"] ? $arParams["TITLE_UPLOAD"] : ($arParams["ALLOW_UPLOAD"] == "I" ? GetMessage("BFDND_UPLOAD_IMAGES") : GetMessage("BFDND_UPLOAD_FILES")) )?></span></a>
		...
		<div class="file-placeholder">
			<table class="files-list" cellspacing="0">
				...
				<?if (is_array($arValue) && sizeof($arValue) > 0)
				{
					...
					{
						?><?=str_replace(
							array("#element_id#", "#name#", "#size#", "#src#"),
							array(intval($arElement['ID']), htmlspecialcharsEx($arElement['ORIGINAL_NAME']), CFile::FormatSize($arElement["FILE_SIZE"]), $arElement['SRC']),
							$thumb
						);
					}
				}?>
