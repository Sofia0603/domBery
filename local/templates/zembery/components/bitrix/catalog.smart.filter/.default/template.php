...
<div class="smart-filter mb-3 <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "smart-filter-horizontal"?>">
    <div class="smart-filter-section">
        ...
        <form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smart-filter-form">
            ...
            <div class="row align-items-center">
                <div class="btn-group smart-filter-group col-12 col-sm mb-3">
                    <?foreach($arResult["ITEMS"] as $key=>$arItem)//prices
                    {
                        $key = $arItem["ENCODED_ID"];
                        if(false && isset($arItem["PRICE"])): // убираем цену из фильтра
                    ...
                    foreach($arResult["ITEMS"] as $key=>$arItem)
                    {
                        ...
                        if ($arItem["DISPLAY_TYPE"] == "A" && ( $arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"] <= 0))
                            continue;
                        ?>
                        <? if($arItem["NAME"] !== 'Преимущества посёлка'):?>
                        <div class="btn-group flex-grow-1 ">
                            <button type="button" class="btn btn-md-lg btn-light dropdown-toggle" data-bs-toggle="dropdown" data-bs-auto-close="outside">
								<span>
									<?=$arItem["NAME"]?>
                                    <? if ($arItem['FILTER_HINT']) echo '<small>('.$arItem['FILTER_HINT'].')</small>' ?>
								</span>
                                <span class="smart-filter-container-modef"></span>
                            </button>

                            <div id="group<?=$arItem['ID']?>" class="dropdown-menu px-2 px-md-3 " >
                                <div class="smart-filter-parameters-box-container">
                                    <?
                ...
                <div class="col-12 col-md-auto mb-3">
                    <input
                            class="btn btn-block btn-md-lg btn-lightgray border"
                            type="submit"
                            id="set_filter"
                            name="set_filter"
                            value="<?=GetMessage("CT_BCSF_SET_FILTER")?>"
                            disabled
                            onclick="ym(89571751,'reachGoal','select'); return true;"
                    />
                    <? /* <input
									class="btn btn-link"
									type="submit"
