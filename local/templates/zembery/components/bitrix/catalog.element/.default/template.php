...
function getColorOfStatusPrice( $arDataItem ) {
    ...
    if ($arDataItem['status'] == 'бронь') return '#FFF3B1';
    if ($arDataItem['status'] == 'благоустройство') return '#B0E5FF';
...
<style>
    ...
    #svg_wrap svg path {
        stroke: black;
        stroke-width: 1px;
        /*d: path("M 1318.93 358.542 L 1242.66 323.068 L 1287.89 228.174 L 1363.27 263.649 L 1318.93 358.542 Z");*/
    }
</style>
<div class="detail-1">
    ...
    <div class="row gx-2">
        <div class="col mb-4">
            <div class="d-flex flex-column flex-sm-row align-items-md-center">
                <h1 class="mb-0 order-1 order-sm-0"><?=trim($arResult['DISPLAY_PROPERTIES']['DETAIL_TITLE'] ? $arResult['DISPLAY_PROPERTIES']['DETAIL_TITLE']['DISPLAY_VALUE'] : $arResult['NAME'])?><a
                            class="ms-3 jsFavorites" href="#" data-id="<?=$arResult['ID']?>"
                            title="Добавить в избранное" onclick="ym(89571751,'reachGoal','add_to_favorites'); return true;" ><i class="fal fa-star font-26"></i></a></h1>
                <? if ($arResult['DISPLAY_PROPERTIES']['PURPOSE']) { ?>
                    <button class="element__purpose-detail ms-sm-4 mb-3 mb-sm-0 order-0 order-sm-1" disabled><?=$arResult['DISPLAY_PROPERTIES']['PURPOSE']['VALUE'] ?></button>
                <? } ?>
            </div>
        ...
        <? if ($arResult['DISPLAY_PROPERTIES']['PRICE_PER_SQUARE']) { ?>
            <div class="col-12 col-lg-auto mb-4 position-relative">
            ...
            <div id="price_subscribe" class="popup" style="display:none;">
                ...
                <div class="darkgray-bg rounded-lg px-3 pt-3">
                    <form class="form-ajax row gx-2 justify-content-center" action="/old/local/include/ajax-price-subscribe.php">
    ...
    <div class="row gx-2 align-items-start border-bottom border-dashed pb-4 mb-4 justify-content-between justify-content-md-start">
        ...
        <div class="col-12 col-sm-6 col-lg-auto mb-3 mb-lg-0 ms-0 ms-lg-auto">
            <a class="btn btn-lg btn-light btn-block jsCompare" href="#" data-id="<?=$arResult['ID']?>" title="Добавить к сравнению" onclick="ym(89571751, 'reachGoal', 'add_to_compare'); return true;">
                <span class="fa-stack fa-4x font-12 me-3">
        ...
        <? if ($arResult['DISPLAY_PROPERTIES']['GEOPOINT']) { ?>
            <div class="col-12 col-sm-6 col-lg-auto">
                <a class="btn btn-lg btn-light btn-block" href="https://yandex.ru/maps/?rtext=~<?=$arResult['DISPLAY_PROPERTIES']['GEOPOINT']['VALUE']?>" target="_blank"><i class="far fa-route me-3"></i>Построить маршрут</a>
            </div>
        <? } ?>
    </div>

    <div class="row justify-content-between align-items-start mb-3">
        <? foreach (array('CITY', 'SHOSSE', 'REGION', 'REMAINDER2', 'REMOTENESS', 'GEOPOINT_SHOT') as $propCode) {
    ...
    <? if ($arResult['DISPLAY_PROPERTIES']['MORE_PHOTO']) { ?>
        <div class="detail-slider">
            <? if ($arResult['DISPLAY_PROPERTIES']['VIDEO_IFRAME']) { ?>
                <div class="detail-slider-item detail-slider-item-video"  >
                    <? echo htmlspecialchars_decode($arResult['DISPLAY_PROPERTIES']['VIDEO_IFRAME']['VALUE']['TEXT'])?>
                </div>
            <? } else if($arResult['DISPLAY_PROPERTIES']['VIDEO']) { ?>

                <a class="detail-slider-item detail-slider-item-video jsScrollTo" data-fancybox="detail" href="<?=$arResult['DISPLAY_PROPERTIES']['VIDEO']['FILE_VALUE']['SRC']?>" >
            ...
            <? foreach ($arResult['DISPLAY_PROPERTIES']['MORE_PHOTO']['FILE_VALUE'] as $arPhotoItem) {
                ...
                ?>
                <a class="detail-slider-item" href="<?=$arPhotoItem['SRC']?>" data-fancybox="detail"><img alt="Земельный посёлок <?=$arResult['NAME']?> <?=$arPhotoItem['ID']?>" class="<?=($isAllSold ? 'img-grayscale':'')?>" src="<?=$img['src']?>" /></a>
            <? } ?>
        </div>
    <? } ?>
</div>
        ...
        <div class="col-12 col-md mb-4">
            ...
            <div class="row gx-2">
                ...
                <? if ($arResult['DISPLAY_PROPERTIES']['REMOTENESS'] || $arResult['DISPLAY_PROPERTIES']['REMOTENESS_TIME']) { ?>
                    <div class="col-auto mb-3">
                        <div class="border rounded-lg p-3 d-flex align-items-center">
                            <i class="far fa-car me-3 font-26"></i>
                            <div class="font-13">
                ...
                <? if ($arResult['DISPLAY_PROPERTIES']['REMOTENESS_RAILWAY']) { ?>
                    <div class="col-auto mb-3">
                        <div class="border rounded-lg p-3 d-flex align-items-center">
                            <i class="far fa-train me-3 font-26"></i>
                            <div class="font-13">
...
<? } ?>
<?php
if (!empty($arResult['PROPERTIES']['IFRAME_VIDEO_CATALOG']["VALUE"]["TEXT"])): ?>
    <?= htmlspecialchars_decode($arResult['PROPERTIES']['IFRAME_VIDEO_CATALOG']["VALUE"]["TEXT"]) ?>
<?php endif; ?>
...
<?php
if (!empty($arResult['PROPERTIES']['SALE_ITEMS']['VALUE'])):?>
    <div class="my-5">
        <h2 class="mb-4">Специальное предложение</h2>
        <div class="specialItems">


            <?php $saleItems = is_array($arResult['PROPERTIES']['SALE_ITEMS']['VALUE']) ? $arResult['PROPERTIES']['SALE_ITEMS']['VALUE'] : [$arResult['PROPERTIES']['SALE_ITEMS']['VALUE']];

            $arFilter = ['IBLOCK_ID' => 24, 'ID' => $saleItems, 'ACTIVE' => 'Y'];
            $arSelect = array("ID", "IBLOCK_ID", 'NAME', 'SALE_BLOCK_TEXT', 'SALE_BLOCK_IMAGE');
            ...
            while ($element = $res->GetNextElement()):
                ...
                $images = $props["SALE_BLOCK_IMAGE"]["VALUE"] ?? []; // Массив ID изображений

                if (!is_array($images)) {
                    $images = [$images]; // Приводим к массиву, если одно изображение
                }
                ?>
                <div class="specialItems-slide  mb-4">
                    <div class="special-block">
                        <div class="specialSlider">
                            ...
                            <?php foreach ($images as $imageID):
                                ...
                                if ($imageSrc): ?>
                                    <a class="slick-slide" href="<?= htmlspecialchars($imageSrc) ?>" data-fancybox="detail">
                                        <img class="special-img" src="<?= htmlspecialchars($imageSrc) ?>" alt="">
                                    </a>
                                <?php endif;
                            endforeach; ?>
                        </div>
                        ...
                        <div class="special-text">
                            <div class="special-text__info">
                                <h4 class="special-text__info-title"><?=$props['SALE_BLOCK_TITLE']['VALUE']?></h4>
                                <ul class="special-text__info-list">
                                    <? foreach (array('CITY', 'SHOSSE', 'REGION', 'REMAINDER2', 'REMOTENESS', 'GEOPOINT_SHOT') as $propCode) {
                                        if ($arResult['DISPLAY_PROPERTIES'][$propCode]) { ?>
                                            <li class="special-text__info-list-item">
                                                <span><?= $arResult['DISPLAY_PROPERTIES'][$propCode]['NAME'] ?>:</span>
                                                <?= $arResult['DISPLAY_PROPERTIES'][$propCode]['DISPLAY_VALUE'] ?> <?= $arResult['DISPLAY_PROPERTIES'][$propCode]['HINT'] ?>
                                            </li>
                                            <?
                                        }
                                    ...
                                    <li class="special-text__info-list-item">
                                        ...
                                        <?= $props['SALE_BLOCK_COST_ALL']['VALUE'] ?>
                                    </li>
                                    <li class="special-text__info-list-item">
                                        <span>Стоимость сотки:</span>
                                        <?= $props['SALE_BLOCK_COST']['VALUE'] ?>
                                    </li>
                                    <li class="special-text__info-list-item">
                                        <span>Площадь участка:</span>
                                        <?= $props['SALE_BLOCK_SQUARE']['VALUE'] ?>
                                    </li>
                                </ul>
                            </div>
                            <!--                                <p>--><?php //= htmlspecialchars_decode($text) ?><!--</p>-->
...
<? if (
    ...
    <div class="wide-gray-bg py-5" >
        <div class="d-flex justify-content-sm-between align-items-center mb-4 tab-items">
            <h2 id="tab-title"></h2>
            <ul class="nav nav-underline tab-items-list" role="tablist" id="maps-switch">
                <li class="nav-item tab-items-list-item" role="presentation">
                    <a class="nav-link active" data-bs-toggle="pill" data-bs-target="#plan" aria-selected="true" role="tab"><?=$arResult['DISPLAY_PROPERTIES']['IMAGE_PLAN']['NAME']?></a>
                </li>
                <li class="nav-item tab-items-list-item" role="presentation">
                    <a class="nav-link" data-bs-toggle="pill" data-bs-target="#3d" aria-selected="false" tabindex="-1" role="tab"><?=$arResult['DISPLAY_PROPERTIES']['TOUR']['NAME']?></a>
                </li>
               <!-- <li class="nav-item tab-items-list-item" role="presentation">
                    <a class="nav-link" data-bs-toggle="pill" data-bs-target="#yndx" aria-selected="false" tabindex="-1" role="tab">Карта посёлка</a>
                </li>-->
                <li class="nav-item tab-items-list-item" role="presentation">
                    <a class="nav-link" data-bs-toggle="pill" data-bs-target="#kadastr" aria-selected="false" tabindex="-1" role="tab"><?=$arResult['DISPLAY_PROPERTIES']['IMAGE_KADASTR']['NAME']?></a>
                </li>
            </ul>
        </div>
                ...
                <div class="tab-pane fade show active" id="plan" role="tabpanel">
                    ...
                    if ( $arResult['DISPLAY_PROPERTIES']['SVG_MAP'] && $arResult['DISPLAY_PROPERTIES']['GDATA'] ) {
                        ...
                        <div class="t_ab-pane f_ade s_how a_ctive" id="plan">
                            <!--                        <h2 class="mb-4 mt-2">--><?php //=$arResult['DISPLAY_PROPERTIES']['IMAGE_PLAN']['NAME']?><!--</h2>-->
                                ...
                                <ul class="nav nav-tabs pt-2 position-absolute ms-2" style="z-index: 1;">
                                    ...
                                    <script>
                                        $(document).ready(function () {
                                            // let elem = document.getElementById('svg_wrap_wrap')
...
//                                    echo '</pre>';
                                        ...
                                        if ($color) {
                                            $replacements[$pathStr] = $pathStr . ' fill="' . $color . '"';
                                        }
                                    }
                                    $svgRaw = preg_replace_callback('/<path id="Vector_(\d+)" (.*?)fill="(\S*)"(.*?)\/>/', function ($matches) use ($replacements) {
                                        $pathStr = '<path id="Vector_' . $matches[1] . '" ';
                                        if (isset($replacements[$pathStr])) {
                                            return $replacements[$pathStr] . $matches[2] . '/>';
                                        }
                                        return $matches[0]; // Если замена не найдена, оставляем как есть
                                    }, $svgRaw);
                        ...
                        <p style="background: #6F787F;">Продан</p>
                        <p style="background: rgb(217,217,217);">Резерв</p>
                        <p style="background: #B0E5FF;">Благоустройство</p>
                        <p style="background: #FFF3B1;">Бронь</p>
                        ';
                            foreach ($arPriceColor as $price => $color) {
                                ...
                                $htmlLegend .= '<p style="background: '.$color.'"> '. $formatPrice . ' за сотку</p>';
                            }
                            $htmlLegend .= '</div>';
                    ...
                    <? } elseif ($arResult['DISPLAY_PROPERTIES']['IMAGE_PLAN']) {
                        ...
                        ?>
                        <div class="t_ab-pane f_ade s_how a_ctive" id="plan">
                            <!--                        <h2 class="mb-4 mt-5">--><?php //=$arResult['DISPLAY_PROPERTIES']['IMAGE_PLAN']['NAME']?><!--</h2>-->
                    ...
                    <? } ?>
                        ...
                        <div class="special-text special-text-hidden">
                            Выберите участок на карте, чтобы узнать подробнее, забронировать или записаться на просмотр
                        </div>
                    <div style="display:none;" class="special-text special-text-no-hidden">
                        <div class="special-text__info">
                                <div id="svg_data_rect2" class="svg_data_rect2" style="display:none;"></div>

                        </div>
                        <div class="special-buttons">
                            <a class="btn btn-lg btn-primary btn-block" href="#book" data-fancybox="" tabindex="0">Забронировать участок</a>
                            <a class="btn btn-lg btn-light btn-block" href="#preview" data-fancybox="" tabindex="0">Запрос на просмотр</a>
                        </div>
                    </div>
                        <!--новый-->
                        <script>
                            $(document).ready(function (){
                                ...
                                $('.special-text-hidden').css('display', 'none');
                                $('.special-text-no-hidden').css('display', 'flex');
                                $('#svg_data_rect2').css('display', 'block');

                                <? if ($arDataItem['price_of'] > 0) { ?>
                                $('#svg_data_rect2').html(
                                    '<h4 class="special-text__info-title">Участок <?=$arDataItem['num']?> <span> <?=$arDataItem['status']?></span></h4>' +
                                    '<ul class="special-text__info-list">' +
                                    '<li class="special-text__info-list-content"> Кадастровый номер: <span><?=$arDataItem['num_kadastr']?></span></li>' +
                                    '<li class="special-text__info-list-content">Площадь участка: <span><?=$arDataItem['volume']?> м2</span></li>' +
                                    '<li class="special-text__info-list-content">Стоимость за сотку: <span><?=CurrencyFormat($arDataItem['price_of'], "RUB")?></span></li>' +
                                    '<li class="special-text__info-list-content">Полная стоимость участка: <span><?=CurrencyFormat($arDataItem['fullprice'], "RUB")?></span></li>' +
                                    '<li class="special-text__info-list-content">Стоимость оформления: <span><?=CurrencyFormat($COST_REGISTRATION_VILLAGE["VALUE"], "RUB")?></span></li>' +
                                    '<li class="special-text__info-list-content">Электронное оформление за 1 день: <span style="white-space: pre;">23 000 ₽</span></li>' +
                                    '</ul>'
                                );
                                <? } else { ?>
                                $('#svg_data_rect2').html(
                                    '<h4 class="special-text__info-title">Участок <?=$arDataItem['num']?> <span> <?=$arDataItem['status']?> </span></h4>' +
                                    '<ul class="special-text__info-list">' +
                                    '<li class="special-text__info-list-content"><?=$arDataItem['price_of']?></li>' +
                                    '<li class="special-text__info-list-content">Площадь участка: <span><?=$arDataItem['volume']?> м2</span></li>' +
                                    '</ul>'
                                );
                                <? } ?>
                        ...
                        <script>
                            $(document).ready(function (){
			                    ...
			                    ?>
                                $('#Vector_<?=$arDataItem['num']?>, [id^="Text_<?=$arDataItem['num']?>_"]').click(function (){
                                    //console.log(<?php //=$arDataItem['num']?>//)
                                    $.fancybox.close();
                                    $.fancybox.open( $('#Vector_<?=$arDataItem['num']?>_popup'), {helpers: { overlay: { locked: false } }} );
                                    return false;
                                }).click(function () {
				                    ...
				                    <? } else{ ?>
                                    $('#svg_data_rect2').html(
                                        '<h4 class="special-text__info-title">Участок <?=$arDataItem['num']?> <span class="village-busy"> <?=$arDataItem['status']?></span></h4>' +
                                        '<ul class="special-text__info-list">' +
                                        '<li class="special-text__info-list-content"> Кадастровый номер: <span><?=$arDataItem['num_kadastr']?></span></li>' +
                                        '</ul>' +
                                        '<div class="special-text__info-busy-text">' +
                                        '<p>Данный участок уже нашел своего покупателя </p>' +
                                        '<p>Наши менеджеры с удовольствием подберут вам другой </p>' +
                                        '</div>'
                                    );
                                    $('#specialButtons').html(
                                        ' <a class="btn btn-lg btn-primary btn-block" href="#other-village" data-fancybox="" tabindex="0">Подобрать другой участок</a>'
                                    );
				                    <? }?>
                                    show();
                                });
            ...
            <div class="tab-pane fade" id="3d" role="tabpanel">
                <? if ($arResult['DISPLAY_PROPERTIES']['TOUR']) { ?>
                    <div class="t_ab-pane f_ade overflow-hidden <?=(false && $isActive ? 'show active':'')?> " id="tour">
                        <!--                        <h2 class="mb-4 mt-5">--><?php //=$arResult['DISPLAY_PROPERTIES']['TOUR']['NAME']?><!--</h2>-->
            ...
            <div class="tab-pane fade" id="kadastr" role="tabpanel">1233
                <? if ($arResult['DISPLAY_PROPERTIES']['IMAGE_KADASTR']) { ?>
                    <div class="t_ab-pane f_ade <?=( false && $isActive ? 'show active':'')?>" id="kadastr">
                        <!--                        <h2 class="mb-4 mt-5">--><?php //=$arResult['DISPLAY_PROPERTIES']['IMAGE_KADASTR']['NAME']?><!--</h2>-->
        ...
        <div id="yndx" role="tabpanel">

		    <? if ($arResult['DISPLAY_PROPERTIES']['GEOINSIDE']) { ?>
                <div id="yndx" class="col-12 <?=($arResult['DISPLAY_PROPERTIES']['ROUTE'] ? 'col-lg-12':'')?> mb-5">
                    <?php $arResult['DISPLAY_PROPERTIES']['GEOINSIDE']['NAME']?></h2>
                    <div class="map-wrap">
		    ...
		    <? } elseif ($arResult['DISPLAY_PROPERTIES']['GEOPOINT']) { ?>
                <div class="col-12 <?=($arResult['DISPLAY_PROPERTIES']['ROUTE'] ? 'col-lg-12':'')?> mb-5">
                                                    <h2 class="mb-4"><?php $arResult['DISPLAY_PROPERTIES']['GEOPOINT']['NAME']?></h2>
                    <div class="map-wrap">
        ...
        <!--<div class="map-wrap">
		    <?php /*=$arResult['DISPLAY_PROPERTIES']['GEOPOINT']['DISPLAY_VALUE']*/?>
        </div>-->
        ...
        if ($arResult['DISPLAY_PROPERTIES']['IMAGE_PLAN'] || $arResult['DISPLAY_PROPERTIES']['SVG_MAP']) { ?>
            <div id="share-popup" class="popup" style="display: none;">
                ...
                <div class="darkgray-bg rounded-lg px-3 pt-3">
                    <form class="form-ajax row gx-2 align-items-center" action="/old/local/include/ajax-share.php">
...
<? } ?>
<?$APPLICATION->IncludeFile('/local/include/inc-calculator.php', array(), array('SHOW_BORDER'=>false, 'MODE' => "html"))?>
<?php
if (!empty($arResult['PROPERTIES']['IFRAME_VIDEO_CATALOG']["VALUE"]["TEXT"])): ?>
    <?= htmlspecialchars_decode($arResult['PROPERTIES']['IFRAME_VIDEO_CATALOG']["VALUE"]["TEXT"]) ?>
<?php endif; ?>
...
<!--    <div id="detail-video" class="py-5">-->
<!--        <video class="w-100" controls >-->
<!--            <source src="--><?php //=$arResult['DISPLAY_PROPERTIES']['VIDEO']['FILE_VALUE']['SRC']?><!--" type='--><?php //=$arResult['DISPLAY_PROPERTIES']['VIDEO']['FILE_VALUE']['CONTENT_TYPE']?><!--'>-->
<!--        </video>-->
<!--    </div>-->
<?// } ?>
<style>
...
<div id="book" class="popup" style="display: none;">
    ...
    <h2>
        Платный резерв участка
    </h2>

    <div class="mb-3">
        <span class="gray"><?=$arResult['NAME']?></span>
    </div>
    <ul class="nav nav-underline mb-4 " role="tablist" id="days-switch">
        <li class="nav-item" role="presentation">
            <a class="nav-link active" href="#days14" data-bs-toggle="pill" aria-selected="true" role="tab" data-days="14">14 дней</a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link" href="#days21" data-bs-toggle="pill" aria-selected="false" tabindex="-1" role="tab" data-days="21">21 день</a>
        </li>
    </ul>
    <div class="darkgray-bg rounded-lg px-3 pt-3">
        <form class="form-ajax row gx-2 justify-content-center" action="/local/include/ajax-bron-create.php">
            ...
            <input type="hidden" name="days" id="daysInput" value="14">
            ...
            <div class="col-12 font-14 text-center mb-3">
                <p class="gray">
                    После успешного заполнения формы, вы автоматически перейдете к этапу оплаты. Обратите внимание, что без завершения этого шага ваша бронь не будет зафиксирована.
                    Пожалуйста, убедитесь, что вы завершили процесс оплаты, чтобы избежать недоразумений.
                </p>
            </div>

            <div class="col-12 col-md-10 font-14  text-center mb-3">
                <p class="gray">Нажимая "Отправить" вы соглашаетесь с <a id="oferta" href="/oferta" target="_blank">офертой</a></p>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    const navLinks = document.querySelectorAll("#days-switch .nav-link");
    ...
    navLinks.forEach(link => {
        link.addEventListener("click", function () {
            ...
            let newHref;
            if(days == 21) {
                newHref = `/oferta${encodeURIComponent(days)}`;
            } else {
                newHref = `/oferta`;
            }
            oferta.href = newHref;
    ...
    var tabs = document.querySelectorAll('#maps-switch .nav-link');
    var title = document.getElementById('tab-title');

    // Устанавливаем заголовок для активной вкладки
    function updateTitle() {
        var activeTab = document.querySelector('#maps-switch .nav-link.active');
        title.textContent = activeTab ? activeTab.textContent : '';
    }

    // Обновляем заголовок при загрузке страницы и при смене вкладки
    updateTitle();

    // Добавляем обработчик событий на изменение активной вкладки
    tabs.forEach(function(tab) {
        tab.addEventListener('click', updateTitle);
    });
</script>
