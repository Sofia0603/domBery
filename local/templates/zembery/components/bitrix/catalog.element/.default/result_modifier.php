...
$arOneLineProp = array();
if ($arResult['DISPLAY_PROPERTIES']['SHOSSE']) {
    $arOneLineProp[] = '<a href="/catalog/filter/shosse-is-' . $arResult['DISPLAY_PROPERTIES']['SHOSSE']['VALUE_XML_ID'] . '/apply/">' . $arResult['DISPLAY_PROPERTIES']['SHOSSE']['DISPLAY_VALUE'] . ' шоссе</a>';
    $arResult['DISPLAY_PROPERTIES']['SHOSSE']['DISPLAY_VALUE'] = '<a href="/catalog/filter/shosse-is-' . $arResult['DISPLAY_PROPERTIES']['SHOSSE']['VALUE_XML_ID'] . '/apply/">' . $arResult['DISPLAY_PROPERTIES']['SHOSSE']['DISPLAY_VALUE'] . '</a>';
}
if ($arResult['DISPLAY_PROPERTIES']['REGION']) {
    $arOneLineProp[] = '<a href="/catalog/filter/region-is-' . $arResult['DISPLAY_PROPERTIES']['REGION']['VALUE_XML_ID'] . '/apply/">' . $arResult['DISPLAY_PROPERTIES']['REGION']['DISPLAY_VALUE'] . ' городской округ</a>';
    $arResult['DISPLAY_PROPERTIES']['REGION']['DISPLAY_VALUE'] = '<a href="/catalog/filter/region-is-' . $arResult['DISPLAY_PROPERTIES']['REGION']['VALUE_XML_ID'] . '/apply/">' . $arResult['DISPLAY_PROPERTIES']['REGION']['DISPLAY_VALUE'] . '</a>';
}
...
if ($arResult['DISPLAY_PROPERTIES']['GEOPOINT']) {
    ...
    $arResult['DISPLAY_PROPERTIES']['GEOPOINT_SHOT'] = array(
        'NAME' => 'Координаты GPS',
        'DISPLAY_VALUE' => '<a href="http://maps.yandex.ru/?ll='.$arCoord[1].'E,'.$arCoord[0].'N&z=14&l=sk,&pt='.$arCoord[1].'E,'.$arCoord[0].'N,pmwtm1" target="_blank">'.round($arCoord[0],6) . ', ' . round($arCoord[1],6) . '<i class="fal fa-crosshairs ms-2 green"></i></a>'
    );
}
...
if ( empty($arResult['DISPLAY_PROPERTIES']['ROUTE']) && ($arResult['DISPLAY_PROPERTIES']['ROUTE_CAR'] || $arResult['DISPLAY_PROPERTIES']['ROUTE_BUS']) ) {
    $arResult['DISPLAY_PROPERTIES']['ROUTE']['NAME'] = $arResult['PROPERTIES']['ROUTE']['NAME'];
    $arResult['DISPLAY_PROPERTIES']['ROUTE']['DISPLAY_VALUE'] = '';

    if ($arResult['DISPLAY_PROPERTIES']['ROUTE_CAR']) $arResult['DISPLAY_PROPERTIES']['ROUTE']['DISPLAY_VALUE'] .= '<h3>На машине</h3>'.$arResult['DISPLAY_PROPERTIES']['ROUTE_CAR']['DISPLAY_VALUE'];
    if ($arResult['DISPLAY_PROPERTIES']['ROUTE_CAR'] && $arResult['DISPLAY_PROPERTIES']['ROUTE_BUS']) $arResult['DISPLAY_PROPERTIES']['ROUTE']['DISPLAY_VALUE'] .= '<br><br>';
    if ($arResult['DISPLAY_PROPERTIES']['ROUTE_BUS']) $arResult['DISPLAY_PROPERTIES']['ROUTE']['DISPLAY_VALUE'] .= '<h3>На общественном транспорте</h3>'.$arResult['DISPLAY_PROPERTIES']['ROUTE_BUS']['DISPLAY_VALUE'];
}
