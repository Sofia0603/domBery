...
<div class="detail-1">
    <div class="row gx-2 align-items-stretch mb-lg-4">
        ...
        <? if ($arResult['DISPLAY_PROPERTIES']['PRICE_FOR_FINISH'] || $arResult['DISPLAY_PROPERTIES']['PRICE_FINISH']) { ?>
            <? if ($arResult['DISPLAY_PROPERTIES']['PRICE_FOR_FINISH']) { ?>
                <div class="col-12 col-sm col-lg-auto mb-4 mb-lg-0 text-sm-right position-relative">
            ...
            <div id="price_subscribe" class="popup" style="display:none;">
                ...
                <div class="darkgray-bg rounded-lg px-3 pt-3">
                        <form class="form-ajax row gx-2" action="/local/include/ajax-price-subscribe.php">
	...
	<div class="row gx-2 align-items-start border-bottom border-dashed pb-4 my-4 justify-content-between justify-content-md-start">
    ...
    if ($arPrice && isset($arPrice['RESULT_PRICE']['DISCOUNT_PRICE'])) {
        $priceProject = $arPrice['RESULT_PRICE']['DISCOUNT_PRICE'];
        $issetPriceProject = true;
    }

    if ($priceProject > 0): ?>
        <div class="col-12 col-sm-6 col-lg-auto mb-3 mb-lg-0">
            <a class="btn btn-lg btn-light btn-block" href="#book-buy" data-fancybox>
                Купить проект <?= number_format($priceProject / 1000, 0, ',', ' ') ?> т.р.
            </a>
        </div>
    <?php endif;
}?>
    ...
    <? if ($arResult['ONE_LINE_PROP']) { ?>
        <div class="row gx-2 align-items-stretch border-bottom border-dashed pb-4 my-4 text-nowrap">
            <?
...
<? if ( $arResult['DISPLAY_PROPERTIES']['HOUSE_PLANS'] || $arResult['DISPLAY_PROPERTIES']['FACADE_PLANS'] ) { ?>
    <div class="wide-gray-bg py-5" >
        <ul class="nav nav-tabs pb-2">
            ...
            <? } ?>
            <? if($issetPriceProject):?>
            <li class="nav-item">
                <a class="nav-link   btn btn-lg btn-light btn-block" href="#book-buy" data-fancybox >Купить проект</a>
            </li>
    <? endif;?>
        </ul>
...
<div id="book-buy" class="popup" style="display: none;">
    ...
    <div class="darkgray-bg rounded-lg px-3 pt-3">
        <form class="form-ajax row gx-2 justify-content-center" action="/local/include/ajax-bron-create.php">
            <input type="hidden" name="form_action" value="buy_project">
            <input type="hidden" name="form_name" value="Покупка проекта <?=$arResult['NAME']?>">
